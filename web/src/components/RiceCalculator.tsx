...
const CONFIDENCE_OPTIONS = [
  { value: 1.0, label: 'Certain', emoji: '🔒', desc: 'There is no realistic way this won’t work.' },
  { value: 0.75, label: 'High', emoji: '✅', desc: 'Highly probable; risk is marginal and backed by data.' },
  { value: 0.5, label: 'Medium', emoji: '🤔', desc: 'Looks promising but we can’t be sure until we try.' },
  { value: 0.25, label: 'Low', emoji: '🎲', desc: 'A long shot — experimental or speculative. No risk, no fun.' },
];
...
function Calculator() {
  ...
  const riceInfo = rice === null ? { label: '—', cls: '' } : rice >= 6 ? { label: 'Must do 🚀', cls: 'must-do' } : rice >= 3 ? { label: 'High priority ✅', cls: 'high' } : rice >= 1.5 ? { label: 'Worth considering 🤔', cls: 'worth' } : { label: 'Low priority ⬇️', cls: 'low' };
        ...
        {step === 4 && (
          <>
            ...
            <div className="rice-score-guide">
              ...
              <p className="must-do">{'🚀 ≥ 6.0 — Must do'}</p>
              <p className="high">{'✅ 3.0–5.9 — High priority'}</p>
              <p className="worth">{'🤔 1.5–2.9 — Worth considering'}</p>
